          ...
          {payments.map(p => (
            <div key={p.id} className="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2"><span className="font-semibold">₹{p.expected_amount}</span><StatusBadge status={p.status} /></div>
                ...
                {p.rejection_reason && <p className="text-sm text-red-600 mt-1">Reason: {p.rejection_reason}</p>}
              </div>
              {p.screenshot_url && <button onClick={() => setSelected(p)} className="btn-secondary flex items-center gap-2 text-sm"><Eye className="h-4 w-4" /> View Screenshot</button>}
            </div>
          ))}
        </div>
      )}
